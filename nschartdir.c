...
NS_EXPORT int Ns_ModuleInit(char *server, char *module)
{
    ...
    Ns_Log(Notice, "nschartdir module version %s server: %s", VERSION,server);
...
static void freeChart(Chart *chart,int lock)
{
    ...
    if(lock) Ns_MutexLock(&chartMutex);
    if(chart->prev) chart->prev->next = chart->next;
    if(chart->next) chart->next->prev = chart->prev;
    if(chart == chartList) chartList = chart->next;
    if(lock) Ns_MutexUnlock(&chartMutex);
    chart->chart->destroy();
    ns_free(chart);
}
...
{
    if(!name) return defalign;
    for(int i = 0;chartAligments[i];i += 2)
     if(!strcasecmp(chartAligments[i],name)) return (Alignment)atoi(chartAligments[i+1]);
    return defalign;
}
...
static int chartColor(Tcl_Interp *interp,Tcl_Obj *obj,int *color)
{
    ...
    if(!strcasecmp(name,"Transparent")) *color = Transparent; else
    if(!strcasecmp(name,"Palette")) *color = Palette; else
    if(!strcasecmp(name,"BackgroundColor")) *color = BackgroundColor; else
    if(!strcasecmp(name,"TextColor")) *color = TextColor; else
    if(!strcasecmp(name,"LineColor")) *color = LineColor; else
    if(!strcasecmp(name,"DataColor")) *color = DataColor; else
    if(!strcasecmp(name,"SameAsMainColor")) *color = SameAsMainColor; else
...
static Chart *createChart(int objc,Tcl_Obj *CONST objv[],Tcl_Interp *interp)
{
    ...
    if(chartList) chartList->prev = chart;
    chartList = chart;
...
static int setPlotArea(Chart *chart,int objc,Tcl_Obj *CONST objv[],Tcl_Interp *interp)
{
    ...
    if(objc < 6 ||
      ...
      Tcl_WrongNumArgs(interp,2,objv,"#chart x y width height ?bgcolor? ?altbgcolor? ?edgecolor? ?hgridcolor? ?vgridcolor?");
      return TCL_ERROR;
    }
...
static int addLegend(Chart *chart,int objc,Tcl_Obj *CONST objv[],Tcl_Interp *interp)
{
    ...
    if(objc < 5 ||
      ...
      Tcl_WrongNumArgs(interp,2,objv,"#chart x y ?vertical? ?bgcolor? ?edgecolor? ?font? ?fontheight? ?fontcolor? ?fontangle? ?align?");
      return TCL_ERROR;
    }
    ...
    if(align) chart->chart->getLegend()->setAlignment(chartAlignment(align,TopLeft));
    if(fontangle) chart->chart->getLegend()->setFontAngle(fontangle);
    return TCL_OK;
}
...
static int addTitle(Chart *chart,int objc,Tcl_Obj *CONST objv[],Tcl_Interp *interp)
{
    ...
    if(objc < 4 ||
      ...
      Tcl_WrongNumArgs(interp,2,objv,"#chart title ?alignment? ?font? ?fontheight? ?fontcolor? ?bgcolor? ?edgecolor? ?border?");
      return TCL_ERROR;
    }
    chart->chart->addTitle(chartAlignment(align),title,font,fontheight,fontcolor)->setBackground(bgcolor,edgecolor,border);
    return TCL_OK;
}
...
static int setBgImage(Chart *chart,int objc,Tcl_Obj *CONST objv[],Tcl_Interp *interp)
{
    ...
    if(objc < 4 ||
       !(image = Tcl_GetStringFromObj(objv[3],0))) {
      Tcl_WrongNumArgs(interp,2,objv,"#chart name ?align? ?-plotarea?");
      return TCL_ERROR;
    }
    if(objc > 4) align = chartAlignment(Tcl_GetStringFromObj(objv[4],0));
    if(objc < 6)
...
static int setColors(Chart *chart,int objc,Tcl_Obj *CONST objv[],Tcl_Interp *interp)
{
    ...
    if(!strcasecmp(name,"transparentPalette")) palette = transparentPalette; else {
      ...
      if(argc > 0) {
        ...
        for(int i = 0;i < argc;i++) Tcl_GetIntFromObj(interp,argv[i],&palette[i]);
      }
    }
    chart->chart->setColors(palette);
    if(argc > 0) ns_free(palette);
    return TCL_OK;
}
    ...
    switch(cmd) {
     ...
     case cmdAddZone: {
         ...
         if(objc < 7 ||
            ...
            Tcl_GetDoubleFromObj(interp,objv[5],&end) != TCL_OK ||
            chartColor(interp,objv[6],&color) != TCL_OK) {
     ...
     case cmdAddMark: {
         ...
         if(objc < 8 ||
           ...
           Tcl_WrongNumArgs(interp,4,objv,"value linecolor linewidth text ?align? ?font? ?fontsize? ?fontcolor? ?fontangle? ?ontop? ?tickcolor?");
           return TCL_ERROR;
         }
...
static int LayerCmd(Chart *chart,int objc,Tcl_Obj *CONST objv[],Tcl_Interp *interp)
{
    ...
    if(cmd > cmdCreate) {
      if(Tcl_GetIntFromObj(interp,objv[4],&layer) != TCL_OK) return TCL_ERROR;
    ...
    switch(cmd) {
     case cmdCreate: {
         ...
         if(objc < 6 ||
            Tcl_ListObjGetElements(interp,objv[5],&argc,&argv) != TCL_OK) {
           Tcl_WrongNumArgs(interp,4,objv,"type data ?name? ?color?");
           return TCL_ERROR;
         }

         double *data = (double*)ns_malloc(argc*sizeof(double));
         for(int i = 0;i < argc;i++) data[i] = atof(Tcl_GetStringFromObj(argv[i],0));
         ...
         if(!strcmp("bar",type)) {
           ...
           if(objc > 6) {
              if(Tcl_ListObjGetElements(interp,objv[6],&namec,&namev) != TCL_OK || namec != argc)
                ...
                for(int i = 0;i < namec;i++) names[i] = Tcl_GetStringFromObj(namev[i],0);
              }
           }
           if(objc > 7) {
             ...
             if(colorc == 1)
               ...
               for(int i = 0;i < colorc;i++) Tcl_GetIntFromObj(interp,colorv[i],&colors[i]);
             }
           }
         ...
         if(!strcmp("trend",type)) {
           ...
           Tcl_AppendResult(interp, "wrong layer type: should be one of line bar scatter area trend hloc candlestick",0);
           ns_free(data);
           return TCL_ERROR;
         }
         ns_free(data);
     ...
     case cmdSetBarGap: {
         ...
         if(chart->layers[layer].type != BarType) break;
     ...
     case cmdSetGapColor: {
         ...
         if(chart->layers[layer].type != LineType) break;
     ...
     case cmdSetDataCombineMethod: {
         int method;
         char *name;
         char *DataCombineMethod[] = { "Overlay", "Stack", "Depth", "Side" , 0};

         if(objc < 6 ||
            !(name = Tcl_GetStringFromObj(objv[5],0))) {
           Tcl_WrongNumArgs(interp,4,objv,"#layer datacombinemethod");
           return TCL_ERROR;
         }
         for(method = 0;DataCombineMethod[method];method++)
           if(!strcasecmp(name,DataCombineMethod[method])) break;
     ...
     case cmdSetDataSymbol: {
         ...
         for(symbol = 0;chartSymbolTypes[symbol];symbol++)
           if(!strcasecmp(symbolName,chartSymbolTypes[symbol])) break;
...
static int dashLineColor(Chart *chart,int objc,Tcl_Obj *CONST objv[],Tcl_Interp *interp)
{
    ...
    if(objc < 5 ||
       chartColor(interp,objv[3],&color) != TCL_OK ||
       Tcl_GetIntFromObj(interp,objv[4],&pattern) != TCL_OK) {
...
static int patternColor(Chart *chart,int objc,Tcl_Obj *CONST objv[],Tcl_Interp *interp)
{
    ...
    if(argc == 1 && Tcl_GetIntFromObj(interp,argv[0],&i) != TCL_OK) {
      Tcl_SetObjResult(interp,Tcl_NewIntObj(chart->chart->patternColor(Tcl_GetStringFromObj(argv[0],0),startx,starty)));
      return TCL_OK;
    }
    ...
    for(i = 0;i < argc;i++) Tcl_GetIntFromObj(interp,argv[i],&pattern[i]);
...
static int gradientColor(Chart *chart,int objc,Tcl_Obj *CONST objv[],Tcl_Interp *interp)
{
    ...
    if(argc == 1 && (name = Tcl_GetStringFromObj(argv[0],0)) &&
      ...
      for(int i = 0;i < argc;i++) Tcl_GetIntFromObj(interp,argv[i],&array[i]);
      Tcl_SetObjResult(interp,Tcl_NewIntObj(chart->chart->gradientColor(array,angle,scale,startx,starty)));
      ns_free(array);
    }
    return TCL_OK;
...
static int PieCmd(Chart *chart,int objc,Tcl_Obj *CONST objv[],Tcl_Interp *interp)
{
    ...
    switch(cmd) {
     case cmdSetData: {
         ...
         double *data = (double*)ns_malloc(argc*sizeof(double));
         for(int i = 0;i < argc;i++) Tcl_GetDoubleFromObj(interp,argv[i],&data[i]);

         if(labelc > 0) {
           labels = (char**)ns_malloc(labelc*sizeof(char*));
           for(int i = 0;i < labelc;i++) labels[i] = Tcl_GetStringFromObj(labelv[i],0);
         }
...
static int ChartCmd(ClientData arg,Tcl_Interp *interp,int objc,Tcl_Obj *CONST objv[])
{
    ...
    case cmdCreate:
        if(!createChart(objc,objv,interp)) return TCL_ERROR;
        break;
